...
const OperationUserServiceGetUserByPage = "/helloworld.v1.UserService/GetUserByPage"
...
func RegisterUserServiceHTTPServer(s *http.Server, srv UserServiceHTTPServer) {
	...
	r.GET("/users/page", _UserService_GetUserByPage0_HTTP_Handler(srv))
}
...
func _UserService_GetUserByPage0_HTTP_Handler(srv UserServiceHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in GetUserByPageRequest
		if err := ctx.BindQuery(&in); err != nil {
			return err
		}
		http.SetOperation(ctx, OperationUserServiceGetUserByPage)
		h := ctx.Middleware(func(ctx context.Context, req interface{}) (interface{}, error) {
			return srv.GetUserByPage(ctx, req.(*GetUserByPageRequest))
		})
		out, err := h(ctx, &in)
		if err != nil {
			return err
		}
		reply := out.(*GetUsertByPageRespone)
		return ctx.Result(200, reply)
	}
}
...
type UserServiceHTTPClient interface {
	...
	GetUserByPage(ctx context.Context, req *GetUserByPageRequest, opts ...http.CallOption) (rsp *GetUsertByPageRespone, err error)
...
func (c *UserServiceHTTPClientImpl) GetUserByPage(ctx context.Context, in *GetUserByPageRequest, opts ...http.CallOption) (*GetUsertByPageRespone, error) {
	var out GetUsertByPageRespone
	pattern := "/users/page"
	path := binding.EncodeURL(pattern, in, true)
	opts = append(opts, http.Operation(OperationUserServiceGetUserByPage))
	opts = append(opts, http.PathTemplate(pattern))
	err := c.cc.Invoke(ctx, "GET", path, nil, &out, opts...)
	if err != nil {
		return nil, err
